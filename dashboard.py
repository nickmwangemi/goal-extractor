...
def fetch_quantifiable_goals(supabase: Client, organization_id: str = None):
    """Fetch quantifiable goals with progress tracking"""
    try:
        result = supabase.schema('peer_progress').table('quantifiable_goals').select('*').execute()
        return result.data if result.data else []
    except Exception as e:
...
def show_quantifiable_goals_tab(supabase: Client):
    ...
    goals = fetch_quantifiable_goals(supabase)
    
    if not goals:
        st.info("📝 No quantifiable goals found.")
        return
    ...
    # Display goals
    for goal in filtered_goals:
        with st.expander(f"🎯 {goal.get('participant_name', 'Unknown')} - {goal.get('goal_text', 'No goal')[:50]}..."):
            col1, col2 = st.columns([2, 1])
            
            with col1:
                st.write(f"**Goal:** {goal.get('goal_text', 'N/A')}")
